...
public class LoadBalancer {
	...
	public double BreakDownCheckTime;
	...
	public static void main(String[] args) throws IOException {
		...
		while(1==1)
		{
			...
			if((Constants.BREAKDOWN_ACTIVATED)&&(loadBalancer.BreakDownCheckTime==loadBalancer.currentTime))
			{
				loadBalancer.BreakDownCheckTime+=1.0/Constants.BREAKDOWN_RATE;
	...
	public void doCalculation()
	{
		...
		System.out.print("Average waiting time: "+avgWaiting);
		System.out.print(" Avgerage Waiting of those who wait: "+avgWaitingOfThoseWhoWait);
		System.out.print(" P0: "+(1.0-utilization));
		System.out.print(" Utilization: "+utilization);
	...
	public int checkProgramTermination()
	{
		int i;
		if(currentRequestToBeChekedInAllRequests<(allRequests.size()-1))
			return 0;
		for(i=0;i<allRequests.size();i++)
		{
			if((allRequests.get(i).status!=Constants.STATUS_DROPPED)&&(allRequests.get(i).status!=Constants.STATUS_FINISHED))
				return 0;
		}
		return 1;
	}
	...
	public double checkNextEventTime()
	{
		...
		for(i=0;i<servers.size();i++)
		{
			...
			{
				t=BrokenServers.get(i).finishRepairingTime-currentTime;
				if(t < minTime)
				{
					minTime=t;
				}
		...
		if(Constants.BREAKDOWN_ACTIVATED)
		{
			t=BreakDownCheckTime-currentTime;
			if(t < minTime)
			{
				minTime=t;
			}
		...
		if(counter==1)
		{
			for(i=0;i<servers.size();i++)
			{
				if(servers.get(i).breakDownStatus==false)
				{
					...
					servers.get(i).breakDownTime=currentTime+RandomVariates.generateWeibullDistribution(Constants.ALPHA, Constants.BETA, Constants.V);
				}
			}
		}
		else if(counter>1)
		...
		for(i=0;i<servers.size();i++)
		{
			if((servers.get(i).breakDownStatus==true)&&(servers.get(i).breakDownTime==currentTime)&&(servers.get(i).status!=Constants.STATUS_BREAKDOWN))
			{
				...
				if(brokenServer.requestBeingExecuted!=null)
				{
					...
					brokenServer.repairingTime=RandomVariates.generateLogNormalDistribution(Constants.LOGNORMAL_MEAN, Constants.LOGNORMAL_STANDARD_DEVIATION);
					if(BrokenServers.size()==0)
					{
